...
export function setupRoutes(app: Express, gameManager: PokerGameManager, supabase: SupabaseClient): void {
  ...
  // Wake idle table - load into memory and start hand if enough players
  // Called by pg_cron via edge function to ensure tables don't stay idle
  app.post('/api/wake-table', async (req: Request, res: Response) => {
    try {
      ...
      logger.info('Wake table request received', { tableId });
      
      // Check if table already in memory
      let table = gameManager.getTable(tableId);
      
      if (table) {
        // Table exists - check if we need to start a hand
        const isPlaying = table.isHandInProgress();
        const playerCount = table.getPlayerCount();
        
        if (isPlaying) {
          return res.json({ 
            success: true, 
            message: 'Table already has active hand',
            handInProgress: true,
            playerCount
          });
        }
        
        // Trigger checkStartHand via forceRecovery
        logger.info('Table in memory but idle - triggering recovery', { tableId, playerCount });
        table.forceRecovery();
        
        return res.json({ 
          success: true, 
          message: 'Triggered hand start on existing table',
          handInProgress: false,
          playerCount
        });
      }
      
      // Table not in memory - load it
      logger.info('Loading table into memory', { tableId });
      table = await gameManager.loadTableIfNeeded(tableId);
      
      if (!table) {
        return res.status(404).json({ 
          success: false, 
          error: 'Table not found or failed to load' 
        });
      }
      
      const playerCount = table.getPlayerCount();
      logger.info('Table loaded successfully', { tableId, playerCount });
      
      return res.json({ 
        success: true, 
        message: 'Table loaded into memory - hand will start automatically',
        playerCount
      });
      
    } catch (error) {
      logger.error('Failed to wake table', { error: String(error) });
      res.status(500).json({ success: false, error: 'Failed to wake table' });
    }
  });
