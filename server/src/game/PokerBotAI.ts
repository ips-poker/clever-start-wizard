...
export function getBotAggression(botName: string): number {
  // Consistent aggression per bot based on name hash
  let hash = 0;
  for (let i = 0; i < botName.length; i++) {
    hash = ((hash << 5) - hash) + botName.charCodeAt(i);
    hash = hash & hash;
  }
  return 30 + Math.abs(hash % 50); // 30-80 range
}
...
export function makeBotDecision(
  ...
  if (phase === 'preflop' || communityCards.length === 0) {
    decision = preflopStrategy(handAnalysis, position, callAmount, pot, stack, isRaised, playersInHand, aggression, bigBlind);
  } else {
    // Postflop strategy
  ...
  // Map 'raise' to 'bet' when there's no current bet
  if (decision.action === 'raise' && currentBet === 0) {
