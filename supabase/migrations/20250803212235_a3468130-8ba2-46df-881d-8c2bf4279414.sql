...
-- Create policies for announcements (simplified without director_id check)
...
BEGIN
  ...
  -- Execute action based on type (removed director_id check for now)
  CASE action_type
    ...
    WHEN 'next_blind_level' THEN
      UPDATE public.tournaments 
      SET current_level = LEAST(current_level + 1, 20),
          last_voice_command = now()
      WHERE id = tournament_id_param;
...
BEGIN
  ...
  result := jsonb_build_object(
    ...
    'current_level', COALESCE(tournament_record.current_level, 1),
