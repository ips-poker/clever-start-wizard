-- Улучшаем функцию redistribute_chips_on_elimination для корректной работы с последним игроком
CREATE OR REPLACE FUNCTION public.redistribute_chips_on_elimination(
...
BEGIN
  ...
  -- Обновляем выбывшего игрока СРАЗУ
  ...
  IF eliminated_chips IS NULL OR eliminated_chips <= 0 THEN
    -- Пересчитываем позиции даже если фишек нет
    PERFORM calculate_final_positions(tournament_id_param);
    RETURN;
  END IF;

  -- Считаем количество активных игроков (ПОСЛЕ обновления статуса)
  ...
  -- Если нет активных игроков (последний игрок), просто выходим
