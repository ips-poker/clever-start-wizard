...
-- Исправляем функцию start_online_tournament_with_seating на 0-based seat numbers
...
DECLARE
  ...
  v_current_seat INTEGER := 0;  -- CHANGED: 0-based indexing
  v_table_ids UUID[] := '{}';
...
BEGIN
  ...
  -- Use players_per_table from tournament settings, default to 6 if not set
  ...
  -- Recalculate to balance players evenly across tables
  ...
  v_current_table := 1;
  v_current_seat := 0;  -- CHANGED: Start from 0
  ...
  LOOP
    ...
    IF v_current_seat >= v_players_per_table THEN  -- CHANGED: >= instead of >
      v_current_seat := 0;  -- CHANGED: Reset to 0
