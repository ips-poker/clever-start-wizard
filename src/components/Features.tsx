...
import { Trophy, BarChart3, Users, Calendar, Shield, Clock, Target, Award, TrendingUp } from "lucide-react";
export function Features() {
  ...
  return <section className="py-12 lg:py-20 bg-accent/50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12 lg:mb-16">
          ...
          <p className="text-base lg:text-xl text-muted-foreground max-w-3xl mx-auto px-4">EPC предоставляет полный спектр услуг для серьезных покерных игроков: от турниров до персональной аналитики</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-8">
          {features.map((feature, index) => {
          const IconComponent = feature.icon;
          return <Card key={`feature-${index}-${feature.title}`} className="group hover:shadow-elegant transition-all duration-300 hover:-translate-y-1 border-border/50">
                 ...
                 <CardContent className="pt-0">
                   ...
                   <ul className="space-y-2">
                     {feature.highlights.map((highlight, idx) => <li key={idx} className="flex items-center text-xs lg:text-sm">
