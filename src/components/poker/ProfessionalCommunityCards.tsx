...
const ProfessionalCard = memo(function ProfessionalCard({
  ...
  const getCardDelay = () => {
    ...
    if (phase === 'flop') {
      // Flop: 3 cards with staggered reveal (index 0, 1, 2)
      return baseDelay + (index * dealDelay);
    } else if (phase === 'turn') {
      // Turn: single card (index 3)
      return baseDelay;
    } else if (phase === 'river') {
      // River: single card (index 4)
      return baseDelay;
    }
  ...
  return (
    <motion.div
      ...
      initial={{ 
        x: phase === 'flop' ? -200 - (index * 30) : 150, 
        y: -80, 
        rotateZ: phase === 'flop' ? -15 + (index * 5) : 20,
        scale: 0.7,
        opacity: 0 
      }}
      animate={{
    ...
    >
      ...
      {!isDealt && (
        <motion.div
          ...
          transition={{ duration: 0.4, delay: (cardDelay + 100) / 1000 }}
          style={{
      ...
      {/* River dramatic glow */}
      {phase === 'river' && !isFlipped && (
        <motion.div
          className="absolute -inset-4 rounded-xl pointer-events-none"
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: [0, 0.5, 0], scale: [0.8, 1.2, 1] }}
          transition={{ duration: 0.8, delay: cardDelay / 1000 + 0.2 }}
          style={{
...
export const ProfessionalCommunityCards = memo(function ProfessionalCommunityCards({ 
  ...
  const preDealDelay = phaseTimings?.preDealDelay ?? 
    (phase === 'flop' ? 500 : phase === 'turn' ? 400 : phase === 'river' ? 400 : 0);
