...
import { POKERSTARS_TIMER, isTimerCritical, isTimerWarning, getTimerColorHex } from '@/constants/pokerTimerConfig';
...
export const PlayerActionTimer = memo(function PlayerActionTimer({ 
  ...
  const isCritical = isTimerCritical(timeLeft);
  const isWarning = isTimerWarning(timeLeft);
...
export const CircularTimer = memo(function CircularTimer({
  ...
  const timerColor = getTimerColorHex(timeRemaining);
