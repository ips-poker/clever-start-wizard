...
// Utility to add rounded corners to an image
export const addRoundedCorners = (canvas: HTMLCanvasElement, radius: number = 20): HTMLCanvasElement => {
  const ctx = canvas.getContext('2d');
  if (!ctx) throw new Error('Could not get canvas context');
  ...
  if (!roundedCtx) throw new Error('Could not get rounded canvas context');
